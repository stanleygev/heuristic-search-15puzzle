...
A PDB stores, for a chosen subset of tiles, the exact number of moves
required to place those tiles at their goal positions, ignoring all
other tiles. Multiple non-overlapping PDBs combine *additively*
(Korf and Felner, 2002) to produce an admissible heuristic.

Supported partitions
--------------------
PARTITION_5_5_5  : 3 tables of 5 tiles each            (~5.7M entries each)
PARTITION_6_5_4  : 6+5+4 tiles                         (lopsided variant)
PARTITION_7_8    : 7 + 8 tiles                         (~half a billion entries; very RAM-heavy)

Notes
-----
* Construction is by backward BFS from the goal pattern. Moving a
  tracked tile increments the stored distance; moving a non-tracked
  tile (i.e. moving the blank past unrelated tiles) costs zero.
* Patterns are stored with the blank position included so the BFS can
  generate successors. The blank's location is not part of the cost
  but disambiguates which pattern is reachable in N moves.
* Tables are pickled to disk after first construction and re-loaded on
  subsequent runs (cheap O(table size) load vs. minutes of BFS).
"""
...
# Useful alias: total tile coverage check
...
class PatternDatabase:
    """Pre-computed exact distances for a chosen tile subset.
    ...
    enumerate successors; it does not contribute to the cost.
    """
    ...
    def build(self, verbose: bool = False) -> None:
        ...
        Pattern layout: (blank_pos, tile_pos_0, tile_pos_1, ...) where
        tile_pos_k is the flat index of the k-th tile in *self.tiles*
        (sorted ascending).
        """
        ...
        while queue:
            ...
            for nb in _NEIGHBOURS[blank_pos]:
                ...
                if nb_tile_idx >= 0:
                    # Moving a tracked tile: cost +1
                    new_pattern = list(pattern)
                    new_pattern[0] = nb
                    new_pattern[nb_tile_idx] = blank_pos
                    ...
                    # Moving a non-tracked tile: cost 0 (blank moves only)
                    new_pattern = list(pattern)
                    new_pattern[0] = nb
                    new_pattern_t = tuple(new_pattern)
                    if new_pattern_t not in self._table:
        ...
        if verbose:
            print(
                f"  PDB for tiles {self.tiles}: {len(self._table):,} entries",
                flush=True,
            )
    ...
    def _state_to_pattern(self, state: tuple[int, ...]) -> tuple[int, ...]:
        """Extract (blank_pos, tile1_pos, ...) from a full 16-tuple state."""
...
class DisjointPDB:
    ...
    verbose :
        Print BFS progress and cache messages.
    """

    def __init__(
        self,
        partition: Sequence[Sequence[int]] = PARTITION_5_5_5,
        cache_dir: str | Path | None = None,
        verbose: bool = False,
    ) -> None:
        # Sanity: partition must be a true partition of {1..15}.
        flat = [t for grp in partition for t in grp]
        assert len(flat) == 15 and frozenset(flat) == ALL_TILES, \
            "Partition must cover tiles 1-15 exactly once."
    ...
    def total_entries(self) -> int:
        return sum(len(p) for p in self._pdbs)

    def num_tables(self) -> int:
        return len(self._pdbs)
